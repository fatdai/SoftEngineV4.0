                                     ...
                                     float zNearPlane, float zFarPlane, Mat4* dst)
{
    ...
    {
        printf("Invalid field of view value (%f) causes attempted calculation tan(%f), which is undefined. \n", fieldOfView, theta);
        return;
    }
...
void Mat4::getColumnX(Vec3* dst){
    dst->x = m[0];
    dst->y = m[1];
    dst->z = m[2];
}
void Mat4::getColumnY(Vec3* dst){
    dst->x = m[4];
    dst->y = m[5];
    dst->z = m[6];
}
void Mat4::getColumnZ(Vec3* dst){
    dst->x = m[8];
    dst->y = m[9];
    dst->z = m[10];
}
...
void Mat4::transformPoint(const Mat4& mat,const Point4& point,Point4* dst){
    assert(dst);
    MathUtil::transformVec4(mat.m,point.x,point.y,point.z, point.w, (float*)dst);
}
